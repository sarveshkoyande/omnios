# Local dev guide â€” running Omni OS + Gemini fallback + inspecting the database
...
Open **http://127.0.0.1:8000/v2** in a browser.

If you changed frontend code (`frontend/src/...`), rebuild first â€” the backend doesn't
...
too â€” `--reload` isn't used here, so nothing hot-reloads.
...
Get a key at https://aistudio.google.com/apikey. Restart uvicorn after changing `.env` â€”
...
1. Azure Foundry configured (`AZURE_AI_FOUNDRY_API_KEY`, or `azure-identity` installed for the
   Azure AD path) â†’ uses Claude via Azure Foundry
2. else `GEMINI_API_KEY` / `GOOGLE_API_KEY` set â†’ uses Gemini
3. else â†’ deterministic rules engine (no AI)
...
installed â€” otherwise the run goes to Foundry and, if that credential fails, straight to the
...
2. Click **+ New plan**.
3. Type a message describing a campaign â€” **use a brand name that is NOT "Oncomyra"**
   (Oncomyra is the one demo brand hardcoded to always use the deterministic rules path,
   regardless of which LLM is configured â€” good for a fast no-AI demo, bad for testing Gemini).
...
5. Once all fields are captured, it kicks off the multi-agent research run (Stage 1 â€”
   deterministic, no LLM involved) and composes the Brand Engagement Plan.
6. Stage tabs across the top (Planning & Strategy â†’ Engagement Orchestration â†’ Campaign
   Operations â†’ Reporting & Insights) unlock once Stage 1 finishes.

## 4. Confirm which engine answered a message

Two ways:

- **Console** â€” the terminal running uvicorn prints a line on every successful LLM turn:
  ```
  [conversation] Gemini answered this turn.
  ```
  (or `Claude (via Microsoft Foundry) answered this turn.` if using Foundry). On failure it
  prints why, and falls back to rules automatically â€” chat never breaks.

- **API directly** â€” call `/api/chat` yourself and check `llm_status` in the response:
  ```powershell
  curl http://127.0.0.1:8000/api/chat -X POST -H "Content-Type: application/json" `
...
- Fully read-only â€” nothing you click can modify data.

**The databases**, all SQLite files under `data/`:

| File | What it stores |
|---|---|
| `projects.db` | Every planning project â€” chat history, captured slots, the composed plan, campaign-ops layout |
| `omni_kb.db` | Scraped knowledge base (ClinicalTrials.gov, PubMed, openFDA, DailyMed, Google Trends) |
| `campaigns.db` | Campaign/content model â€” brands, segments, messages, claims, content assets, KPIs |
...
(Cognee's own grounding store lives at `data/cognee/` â€” not a plain SQLite file, not
shown in the viewer.)

## 6. Cleaning up test data

Projects created while testing show up in `projects.db` â†’ `projects` table. To delete one:
...
- `main` â€” up to date with GitHub as of the last pull.
- `llm-gemini-fallback` â€” rebased on top of latest `main`. Contains only the
  Gemini-priority LLM change. **Not pushed to GitHub yet** â€” stays local until you say so.
