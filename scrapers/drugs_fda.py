"""Drugs@FDA downloadable data-file scraper.

Downloads the public Drugs@FDA ZIP, stores the raw archive and each tab-delimited text
table, and indexes the extracted files in the document lake. Structured loading lives in
`pharma_intel.py`.
"""
...
USER_AGENT = "OmniDataHubResearchBot/0.1 (+local research; Drugs@FDA public data)"
HEADERS = {
    "User-Agent": USER_AGENT,
    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,text/plain;q=0.8,*/*;q=0.5",
}
FALLBACK_ZIP_URLS = [
    "https://www.fda.gov/downloads/Drugs/InformationOnDrugs/UCM054599.zip",
...
def _discover_zip_url(page_html: str) -> str | None:
    ...
    for href, text in parser.links:
        ...
        if "zip" in candidate or "download file" in candidate or "drugs@fda download" in candidate:
...
def _download_zip() -> tuple[str, bytes]:
    ...
    raise RuntimeError(f"could not download Drugs@FDA ZIP ({last_error})")
...
def run() -> int:
    ...
    try:
        ...
        zip_blob = save_blob("drugs_fda", f"drugs_fda_{digest}.zip", payload)
        upsert_document(
            conn,
            source="drugs_fda:zip",
            external_id=digest,
            search_term="Drugs@FDA",
            title="Drugs@FDA downloadable data ZIP",
            doc_type="drugs_fda_zip",
            url=url,
            blob_path=zip_blob,
        ...
        with zipfile.ZipFile(io.BytesIO(payload)) as zf:
            for member in zf.namelist():
                ...
                blob = save_blob("drugs_fda", f"{digest}_{member}", text)
                normalized_member = re.sub(r"[^a-z0-9]+", "_", member.lower()).strip("_")
                upsert_document(
                    conn,
                    source=f"drugs_fda:{normalized_member}",
                    external_id=member_id,
                    search_term="Drugs@FDA",
                    title=f"Drugs@FDA {member}",
                    doc_type="drugs_fda_data_file",
                    url=url,
                    blob_path=blob,
                    metadata={"zip_sha1": digest, "member": member, "rows_hint": max(0, text.count("\n") - 1)},
                )
                count += 1
        print(f"[drugs_fda] saved {count} documents from {url}")
        time.sleep(0.5)
        return count
    finally:
        conn.close()
